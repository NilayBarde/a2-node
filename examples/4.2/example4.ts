...
function driver(promiseName: string, flag: boolean) {
    ...
    makePromise1(promiseName, flag, 10)
        .then((n) => {
            console.log(
                `promise ${promiseName} fulfilled and passed ${n} to its successor`
            );
            console.log(
                `the then block of ${promiseName} will now throw an error`
            );
        ...
        .catch((n) =>
            console.log(
                `promise ${promiseName} rejected and passed "${n}" to its successor`
            )
        );
}
